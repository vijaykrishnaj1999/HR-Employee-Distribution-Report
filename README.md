# HR-Employee-Distribution-Report
This dashboard analyzing HR workforce demographics : gender, race, age, department, location HQ vs Remote, and 20 year employee growh growth tends
<img width="1302" height="740" alt="image 1" src="https://github.com/user-attachments/assets/e595f7c5-cd92-48de-aaad-f41ef919663f" />
<img width="1312" height="735" alt="image 2" src="https://github.com/user-attachments/assets/562928a3-a42b-4e42-bc37-c3dc6c793da8" 

*Description*
Power BI dashboard analyzing HR workforce demographics: gender, race, age, department, location HQ vs Remote, and 20-year employee growth trends
HR Analytics dashboard built with Power BI to analyze employee distribution and workforce demographics. Covers gender, age, race, department, location, and employment trends from 2000-2020 to support DEI and workforce planning decisions.

*Page 1: HR Employee Distribution Report*
1. *Gender Distribution*: Male 8.9K, Female 8.1K, Non-Conforming 0.5K
2. *Headquarters vs Remote*: 74.97% HQ, 25.03% Remote
3. *Average Length of Employment*: 7 Years
4. *Employee Growth 2000-2020*: Line chart showing growth from 82% to 98%
5. *Employees By State*: Map focused on Ohio, Pennsylvania, Kentucky, Michigan, Illinois, Wisconsin
6. *Race Distribution*: White 5.0K, Two or More Races 2.9K, Black 2.8K, Asian 2.8K, Hispanic 2.0K

*Page 2: Age & Department Analysis*
1. *Age Distribution by Gender*: Peak in 25-34 and 35-44 age groups
2. *Age Group Distribution*: 5.0K in 25-34, 4.9K in 35-44, 4.6K in 44-54
3. *Termination Rate by Department*: Highest in Auditing 0.16, Legal 0.12
4. *Gender Distribution by Department*: Engineering and Accounting have highest headcount

*Tools*: Power BI Desktop, DAX, Power Query
